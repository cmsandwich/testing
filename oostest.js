window.gibinit = function() {
  ...
  function detectLocationId() {
    if (window.gibRetailer == 'target') {
      var locationId = document.getElementByID(shopping_location_id);
  ...
  function target_addToCart(DEBUG_NODE, UPC) {
    var cartUrl = 'https://carts.target.com/web_checkouts/v1/cart_items?field_groups=CART%2CCART_ITEMS%2CSUMMARY%2CFINANCE_PROVIDERS&key=feaf228eb2777fd3eee0fd5192ae7107d6224b39';
    var cart = {"cart_type":"REGULAR","channel_id":"10","shopping_context":"DIGITAL","cart_item":{"tcin":UPC,"quantity":1,"item_channel_id":"10"},"fulfillment":{"type":"PICKUP","location_id": locationId,"ship_method":"STORE_PICKUP"}};
  ...
  function startWatcher(GIB, DEBUG_NODE) {
    if (!window.gibSupportedSites.includes(window.gibRetailer)) {
      DEBUG_NODE.innerText = 'Retailer not supported! This tool does not currently support ' + window.gibRetailer + '. Follow @GroupOos for the latest supported retailers.';
      window.gibWatcherDisable();
      window.gibSoundTrigger(5, true);
      return false;
    }
